...
def is_panda_aware(filename):
    ...
    if pypanda_start_pattern in contents:
        if pypanda_end_pattern not in contents:
...
def trim_pypanda(contents):
    ...
    return contents[a:b]+recurse if recurse else contents[a:b]
...
def copy_ppp_header(filename):
    ...
    print(f"Creating pypanda PPP header [{pypanda_h}] for [{filename}]")
...
def create_pypanda_header(filename, no_record=False):
    ...
    for line in subcontents.split("\n"):
        ...
        if foo:
            ...
            print(f"Found nested include of {nested_inc}")
            create_pypanda_header(f"{plugin_dir}/{nested_inc}")
        else:
            rest.append(line)
    new_contents = "\n".join(rest)
    ...
    pypanda_h = f"{os.path.join(INCLUDE_DIR_PYP, foo.groups()[0])}.h"
    print(f"Creating pypanda header [{pypanda_h}] for [{filename}]")
...
def read_but_exclude_garbage(filename):
    ...
    with open(filename) as thefile:
        for line in thefile:
            keep = True
            if re.search("^\s*\#", line) and not re.search(
                "^\s*\#define [^_]", line
            ):
                keep = False
            if keep:
        ...
        if "PACKED_STRUCT" in subcontents: # Replace PACKED_STRUCT(foo) with foo. For kernelinfo.h
            for line in subcontents.split("\n"):
                if "PACKED_STRUCT" in line:
                    struct_name = re.search(r'PACKED_STRUCT\(([a-zA-Z0-9_-]*)\)', line)[1]
